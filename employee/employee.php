...
if ($_SESSION['role'] !== 'Employee') {
    header("Location: ../index.php"); // Redirect non-Employee users to the home page.
    exit();
}
include('../db.php');
...
// Your existing queries, now adjusted based on the logged-in user
$totalSalaryQuery = "SELECT SUM(total_price) as total_salary FROM salary WHERE user_id = '$userId'";
$todayDate = date('Y-m-d');
$yesterdayDate = date('Y-m-d', strtotime('-1 day'));

// Today's queries
$totalSalaryTodayQuery = "SELECT SUM(total_price) as total_salary_today FROM salary WHERE user_id = '$userId' AND DATE(created_at) = '$todayDate'";
$totalBaseSalaryTodayQuery = "SELECT SUM(base_price) as total_base_salary_today FROM salary WHERE user_id = '$userId' AND DATE(created_at) = '$todayDate'";

// Yesterday's queries
$yesterdayIncomeQuery = "SELECT SUM(total_price) as yesterday_income FROM salary WHERE user_id = '$userId' AND DATE(created_at) = '$yesterdayDate'";
$yesterdayBaseSalaryQuery = "SELECT SUM(base_price) as yesterday_base_salary FROM salary WHERE user_id = '$userId' AND DATE(created_at) = '$yesterdayDate'";

// Previous month's total salary
$lastMonthStart = date('Y-m-01', strtotime('first day of last month'));
$lastMonthEnd = date('Y-m-t', strtotime('last day of last month'));
$lastMonthSalaryQuery = "SELECT SUM(total_price) as last_month_salary FROM salary WHERE user_id = '$userId' AND DATE(created_at) BETWEEN '$lastMonthStart' AND '$lastMonthEnd'";

// Last 7 days salary
$last7DaysStart = date('Y-m-d', strtotime('-7 days'));
$last7DaysSalaryQuery = "SELECT SUM(total_price) as last_7_days_salary FROM salary WHERE user_id = '$userId' AND DATE(created_at) BETWEEN '$last7DaysStart' AND '$todayDate'";

// Previous 7 days salary (from 14 to 8 days ago)
$previous7DaysStart = date('Y-m-d', strtotime('-14 days'));
$previous7DaysEnd = date('Y-m-d', strtotime('-8 days'));
$previous7DaysSalaryQuery = "SELECT SUM(total_price) as previous_7_days_salary FROM salary WHERE user_id = '$userId' AND DATE(created_at) BETWEEN '$previous7DaysStart' AND '$previous7DaysEnd'";
...
$last7DaysSalary = $conn->query($last7DaysSalaryQuery)->fetch_assoc()['last_7_days_salary'] ?? 0;
$previous7DaysSalary = $conn->query($previous7DaysSalaryQuery)->fetch_assoc()['previous_7_days_salary'] ?? 0;
...
// Last 7 days trend
$last7DaysDifference = $last7DaysSalary - $previous7DaysSalary;
$last7DaysTrend = $last7DaysDifference >= 0 ? 'up' : 'down';

// Хэрэглэгчийн нийт ажилласан хоногийг тоолох
$activeDaysQuery = "SELECT COUNT(DISTINCT DATE(created_at)) AS active_days 
                    FROM jobs 
                    WHERE user_id = '$userId'";

$activeDays = $conn->query($activeDaysQuery)->fetch_assoc()['active_days'] ?? 0;

// Нийт хугацааг тооцох
$totalDaysQuery = "SELECT DATEDIFF(NOW(), MIN(created_at)) AS total_days 
                   FROM jobs 
                   WHERE user_id = '$userId'";

$totalDays = $conn->query($totalDaysQuery)->fetch_assoc()['total_days'] ?? 0;

// Идэвхгүй хоногийг нийт хугацаанаас идэвхтэй хоногийг хасаж тооцно
$inactiveDays = max(0, $totalDays - $activeDays);

// Идэвхгүй хоногийг сар, хоногоор хувиргах
$inactiveMonths = floor($inactiveDays / 30); // Бүтэн саруудыг тооцно
$remainingDays = $inactiveDays % 30; // Үлдсэн хоногийг тооцно

// Хоногийн текстийг үүсгэх
if ($inactiveMonths > 0) {
    $inactiveText = "$inactiveMonths сар ";
    if ($remainingDays > 0) {
        $inactiveText .= "$remainingDays хоног ";
    }
    $inactiveText .= "ажилаагүй";
} else {
    $inactiveText = "$inactiveDays хоногийн өмнө ажилласан";
}




// Fetch data for job search (optional)
...
$sql = "SELECT jobs.id, users.full_name AS user_name, services.service_name, services.car_type, 
        ...
        WHERE jobs.user_id = '$userId' AND DATE(jobs.created_at) = CURDATE() 
...
// Fetch total number of services for pagination
$totalSql = "SELECT COUNT(*) as total_jobs FROM jobs WHERE user_id = '$userId' AND DATE(created_at) = CURDATE()";
if ($searchQuery != '') {
    $totalSql .= " AND (vehicle_number LIKE '%" . $conn->real_escape_string($searchQuery) . "%' OR payment LIKE '%" . $conn->real_escape_string($searchQuery) . "%')";
}
  ...
  <section class="home-section">
    ...
    <div class="box">
        <div class="right-side">
            ...
            <div class="number">₮<?php echo number_format(abs($totalSalary)); ?></div>
            <div class="indicator">
        ...
        <i class="fa-solid fa-money-bill cart" style="color: #74C0FC;"></i>
    </div>
    
  
    <div class="box">
        <div class="right-side">
            <div class="box-topic">7 хоногийн нийт орлого</div>
            <div class="number">₮<?php echo number_format(abs($last7DaysDifference)); ?></div>
            <div class="indicator">
                <i class='bx <?php echo $last7DaysTrend === "up" ? "bx-up-arrow-alt" : "bx-down-arrow-alt down"; ?>'></i>
                <span class="text">
                    <?php 
                    echo $last7DaysTrend === "up" 
                        ? "Өмнөх 7 хоногоос ₮" . number_format(abs($last7DaysDifference)) . " өссөн" 
                        : "Өмнөх 7 хоногоос ₮" . number_format(abs($last7DaysDifference)) . " буурсан"; 
                    ?>
                </span>
            </div>
        </div>
    ...
    <div class="box">
    <div class="right-side">
    <div class="box-topic">Нийт ажилласан хоног</div>
    <div class="number"><?php echo $activeDays; ?></div>
    <div class="indicator">
        <i class='bx <?php echo $inactiveDays === 0 ? "bx-up-arrow-alt" : "bx-down-arrow-alt down"; ?>'></i> 
        <span class="text">
            <?php 
            echo $inactiveDays === 0 ? "$activeDays хоног идэвхтэй ажилласан" : $inactiveText; 
            ?>
        </span>
    </div>
</div>

<i class="fa-solid fa-calendar-week cart two" style="color: #63E6BE;"></i></div>
        ...
        if ($topSalaryResult->num_rows > 0) {
            while ($row = $topSalaryResult->fetch_assoc()) {
                ...
                if (mb_strlen($full_name) > 15) {
                    ...
                    $display_name = mb_substr($full_name, 0, 1) . '..';
                } else {
                ...
                if ($total_salary >= 1000) {
                    // Format salary to show with "k" suffix
                    $formatted_salary = number_format($total_salary / 1000, 1) . 'k';
                } else {
                    // If less than 1000, show normally
                    $formatted_salary = number_format($total_salary);
                }
        
                echo '</a>';
                
                echo '<span class="price">₮' . $formatted_salary  . '</span>';
